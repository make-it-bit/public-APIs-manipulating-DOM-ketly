...
const submit_button = document.getElementById('first-submit');

submit_button.addEventListener('click', () => {
  const formData = {};

  const coctailValue = document.getElementById('cocktail').value;
  ...
  formData.cocktail = coctailValue;

  if (coctailValue === 'by-name') {
    // ...
  }
...
const fetch_data = async (input_value, index_of_choice) => {
  try {
    ...
    if (index_of_choice === 1) {
      response = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${input_value}`);
    } else if (index_of_choice === 2) {
      response = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/search.php?i=${input_value}`);
